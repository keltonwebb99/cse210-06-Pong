...
class SceneManager:
    ...
    def _add_score(self, cast):
        ...
        label1 = Label(text, SCORE_1_POSITION)
        cast.add_actor(SCORE_GROUP, label1)
        label2 = Label(text, SCORE_2_POSITION)
        cast.add_actor(SCORE_GROUP, label2)

    def _add_stats(self, cast):
        cast.clear_actors(STATS_GROUP)
        stat1 = Stats()
        cast.add_actor(STATS_GROUP, stat1)
        #Add something for player2
        stat2 = Stats()
        cast.add_actor(STATS_GROUP, stat2)
    ...
    #     cast.clear_actors(RACKET_GROUP)
    #     x = CENTER_X - RACKET_WIDTH / 2
    #     y = SCREEN_HEIGHT+300 - RACKET_HEIGHT
    #     position = Point(x, y)
    #     size = Point(RACKET_WIDTH, RACKET_HEIGHT)
    #     velocity = Point(0, 0)
    #     body = Body(position, size, velocity)
